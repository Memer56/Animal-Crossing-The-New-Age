...
const SAVE_GAME_PATH_JSON = "user://Game Data/save_ids"
...
func write_savegame_data(id : String):
	var save_path = str(SAVE_GAME_PATH_PLAYER + id + " " + game_version + '.tres')
...
static func delete_savegame(id : String):
	var game_version = ProjectSettings.get_setting("application/config/version")
	var save_path_player = str(SAVE_GAME_PATH_PLAYER + id + " " + game_version + '.tres')
	var save_path_buildings = str(SAVE_GAME_PATH_BUILDINGS + id + " " + game_version + '.tres')
	DirAccess.remove_absolute(save_path_player)
	DirAccess.remove_absolute(save_path_buildings)

## Functions to save building data
func write_savegame_data_buildings(id : String):
	var save_path = str(SAVE_GAME_PATH_BUILDINGS + id + " " + game_version + '.tres')
	ResourceSaver.save(self, save_path)

static func save_exists_buildings(id : String):
	var game_version = ProjectSettings.get_setting("application/config/version")
	var save_path = str(SAVE_GAME_PATH_BUILDINGS + id + " " + game_version + '.tres')
	return ResourceLoader.exists(save_path)

static func load_savegame_data_buildings(id : String):
	var game_version = ProjectSettings.get_setting("application/config/version")
	var save_path = str(SAVE_GAME_PATH_BUILDINGS + id + " " + game_version + '.tres')
	return ResourceLoader.load(save_path, "", 2)
